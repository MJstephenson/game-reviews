...
def manage_reviews():
    # convert the user id back to ObjectId so that it matches the type of the
    # id
    current_user_id = ObjectId(session["user_id"])
    # only show the reviews from the current active user in session by their id
...
def delete_review(
    review_id,
):  # This will grab the ID of the review I want to delete from my database
    mongo.db.add_review.find_one_and_delete(
        {"_id": ObjectId(review_id)}
    )  # This will delete the entire (one) review in my database
...
def register():
    if request.method == "POST":
        ...
        # store the user_id in the apps session
...
def profile(username):
    ...
    review_count = mongo.db.add_review.count_documents(
        {"user_id": ObjectId(session["user_id"])}
    )
    # see how many reviews the user has in their sessin and display the game
    # names
    ...
    if request.method == "POST":
        ...
        if 3 <= len(new_author) <= 30:
            ...
            flash(
                "You have successfully updated your author name."
            )  # Flash my success message
        else:
            flash(
                "The author name should be between 3 and 20 characters long."
            )  # Flash my error message
...
@app.route("/logout")
def logout():
    # this will remove user and the user_id from session cookies when they log
    # out.
...
def add_review():
    ...
    if request.method == "POST":
        ...
        if existing_review:
            error_message = ("Just to let you know you "
                             ...
                             "Edit Your current one in "
                             "the manage reviews page.")
        else:
    ...
    return render_template(
        "add_review.html", game_names=game_names, error_message=error_message
    )
...
def edit_review(review_id):
    if request.method == "POST":  # get my form data
        ...
        mongo.db.add_review.update_one(
            {"_id": ObjectId(review_id)},
            {
                "$set": {  # Updates the review in my database
                    "game_name": game_name,
                    "publisher": publisher,
                    "developer": developer,
        ...
        )  # redirects back to my manage_reviews.html

    else:
        # Gets the requirde review to edit from my database
        review_to_edit = mongo.db.add_review.find_one(
            {"_id": ObjectId(review_id)})
...
def reviews():
    ...
    review_count = mongo.db.add_review.count_documents({})
    return render_template(
        "reviews.html", add_review=add_review, review_count=review_count
    )
